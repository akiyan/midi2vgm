...
- それ以外の MIDI チャンネルは GM プログラム番号で `piano`、`bell`、`organ`、`guitar`、
  `bassgtr`、`strings`、`brass`、`softlead`、`clar`、`flute`、`synthlead`、`pad`、`pluck`
  のいずれかへ分類します。
...
- `mul`: 倍音比。bell、piano、guitar、pluck では高めの倍音を短く鳴らして打鍵感を作ります。
...
- `rs`: レートスケーリング。高音ほど減衰が速くなるため、piano、guitar、pluck の高音が長く残りすぎるのを抑えます。
