...
含めません。手元で使用権のある MIDI を `assets/midi/` に置いて使います。
...
- 曲別の調整ファイル `assets/midi/adjustments.txt`
- PSG 専用の手動ルーティング `assets/vgm/psg.txt`
...
cd midi2vgm
mkdir -p out
cp /path/to/your_song.mid assets/midi/your_song.mid
python3 scripts/genvgm.py assets/midi/your_song.mid out/your_song.vgm --pcm-drums --atten=6
```

PCM を使わず、FM6 まで BGM に使う場合:

```sh
python3 scripts/genvgm.py assets/midi/your_song.mid out/your_song_fm6.vgm --fm6 --atten=6
```

PSG 専用 VGM を作る場合:

```sh
python3 scripts/genpsg.py assets/midi/your_song.mid out/your_song_psg.vgm \
...
scripts/
  ...
  genallbgm.py    batch wrapper for assets/midi/*.mid -> res/bgm/*.vgm
  genpsg.py       MIDI -> SN76489-only VGM
  genallpsg.py    batch wrapper using assets/vgm/psg.txt
  psg_voice.py    shared PSG decay/vibrato/octave-folding helpers
assets/midi/
  adjustments.txt per-song options for genallbgm.py
...
```sh
python3 scripts/genvgm.py assets/midi/02_introduction.mid res/bgm/02_introduction.vgm \
  --pcm-drums --bell-psg --pcm-shot=auto+4 --atten=6

python3 scripts/genvgm.py assets/midi/03_gerende_no_koibitotachi.mid res/bgm/03_gerende.vgm \
  --fm6 --atten=6
```

## Batch Conversion

`scripts/genallbgm.py` expects MIDI files under `assets/midi/` and writes VGM files
to `res/bgm/`.

```sh
python3 scripts/genallbgm.py
python3 scripts/genallbgm.py 02_introduction
```

The current batch list is in `scripts/genallbgm.py`. Per-song flags are read from
`assets/midi/adjustments.txt`.
...
```sh
python3 scripts/genallpsg.py
python3 scripts/genallpsg.py 02_introduction
```

`assets/vgm/psg.txt` format:
...
project with licensed MIDI assets, keep those assets in that private project or
copy them locally after cloning this repository.
