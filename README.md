...
- ch10 のプログラムチェンジを SoundFont のドラムキット（Standard / Room / Power / 808 / Jazz など）へ自動対応し、曲中のキット切替も追従
- ドラムのベロシティを数段階に量子化し、強弱を PCM に反映（`--drum-vel-levels`）
...
- PCM drums
  ...
  ch10 のプログラムチェンジは GS / GM 流にドラムキット選択として解釈し、SoundFont の
  bank128 プリセット（Standard / Room / Power / Electronic / TR-808 / Jazz / Brush / Orchestra など）の
  うち最も近いものへ自動スナップします。曲の途中でキットが切り替わる場合も発音時点のキットに追従します。
- `--drum-vel-levels=N`
  ドラムのベロシティを `N` 段階（1〜8、既定 4。1 はベロシティ無効）に量子化します。各段は別々の
  PCM サンプルとして収録するため、段数を増やすほど強弱の表情は豊かになりますが出力容量も増えます。
  実際に曲中で出現した段だけを収録するので、ベロシティが一様な曲では段数を上げても容量は増えません。
- `--psg-drums`
